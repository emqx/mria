...
t_rebalance(_) ->
    ...
    ?check_trace(
       ...
       try
           ...
           %% 3. Restart the core and make sure all replicants recognize
           %% it as the most desirable upstream:
           N2 = mria_ct:start_slave(mria, C2),
           mria_ct:start_mria(C2),
           ?retry(1000, 10,
                  [?assertMatch({ok, N2},
                                get_preferred_core_node(test_shard, N))
                   || N <- [N3, N4]]),
           %% 4. Check that rebalance functions work fine when the
...
get_preferred_core_node(Shard, Replicant) ->
    ?ON(Replicant,
        begin
            mria_lb ! update,
            logger:notice("Replicant's internal status ~p", [sys:get_state(mria_lb)]),
