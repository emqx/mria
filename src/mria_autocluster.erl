...
-define(LOG(Level, Format, Args),
        logger:Level("Mria(AutoCluster): " ++ Format, Args)).

-spec(enabled() -> boolean()).
enabled() ->
    case mria:env(cluster_discovery) of
        {ok, {manual, _}} -> false;
        {ok, _Strategy}   -> mria_rlog:role() =:= core;
        undefined         -> false
    end.
...
wait_application_ready(App, Retries) ->
    case mria_node:is_running(App) of
        true  -> ok;
...
strategy_module(Strategy) ->
    case code:is_loaded(Strategy) of
        ...
        false     -> list_to_atom("mria_cluster_" ++  atom_to_list(Strategy))
    end.

discover_and_join(Mod, Options) ->
    case Mod:discover(Options) of
        {ok, Nodes} ->
            maybe_join([N || N <- Nodes, mria_node:is_aliving(N) andalso
